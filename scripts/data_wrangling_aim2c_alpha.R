...
curr_import_folder = 'original_data/Aim2b/alpha/'
...
for (iSubj in 1:length(all_subj)){
	...
	subj_dat = NULL
		...
		curr_dat = read.table(curr_file, header=T, sep="\t", skip=1, fill=T)
		...
		# define condition
		cond_freq = str_locate(FileName, "__")[1] # locates the position of patterns within a string. the string __ is always present in each file, and is a good anchor to retrieve condition name
		cond = substr(FileName, 1, cond_freq-7)
		freq = substr(FileName, 5, cond_freq-1)
		...
		curr_dat$Freq = as.factor(freq)
		
		## Adjust some variables
		curr_dat$X=NULL
		#cn(curr_dat)
		curr_dat_long = rearrange(curr_dat, deps = c(1:70), oth=c(73:76), name.dep="Pow", name.newvar="Electrode")
		
		curr_dat_avg = aggregate(Pow ~ Electrode + ID + cond + Freq, data = curr_dat_long[curr_dat_long$Time>timewin[1] & curr_dat_long$Time<timewin[2],], FUN=mean)
...
save(dat, file = paste(curr_export_folder, "dat_aim2c_alpha_old_new.RData", sep=""))
