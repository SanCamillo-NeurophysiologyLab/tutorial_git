...
curr_import_folder = 'original_data/Aim1/'
...
for (iSubj in 1:length(all_subj)){
	...
	subj_dat = NULL
		...
		cond = substr(FileName, 1, cond_subj-6)
		...
		curr_dat_avg = aggregate(Ampl ~ Electrode + ID + cond, data = curr_dat_long[curr_dat_long$Time>N1_timewin[1] & curr_dat_long$Time<N1_timewin[2],], FUN=mean)
...
save(dat, file = paste(curr_export_folder, "dat_aim1_man_nat.RData", sep=""))
